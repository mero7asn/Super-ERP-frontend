    ...
    <div>
      <div className="page-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
        <div>
          ...
          <p className="page-subtitle">Plan and execute cycle counts by ABC classification</p>
        </div>
        <button className="btn btn-primary" onClick={() => setShowForm(true)} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <Icon name="plus" size={16} /> New Cycle Count
        </button>
      </div>
        ...
        <div className="card" style={{ padding: 0 }}>
          <div className="table-wrapper" style={{ overflowX: 'auto' }}>
            <table className="table">
              ...
              <tbody>
                {counts.length === 0 ? (
                  <tr><td colSpan="8" style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>No cycle counts yet.</td></tr>
                ) : counts.map(cc => (
                  <tr key={cc._id}>
                    <td style={{ fontFamily: 'monospace', fontWeight: 600 }}>{cc.countId}</td>
                    <td>{cc.warehouse?.code}</td>
