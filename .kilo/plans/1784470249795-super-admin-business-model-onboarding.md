...
ERP departments: inventory core, supply chain core), or does **Both** (sees everything). The
...
- **External departments** = two new sidebar sections at the **ERP level, outside Core 360**:
  `inventory core` and `supply chain core`. Shown only for `product` / `both`.
...
- `frontend/src/components/Sidebar.jsx` renders three groups: Global, **Core 360** (`CRM_NAV_ITEMS`),
  **HRM core** (`HRM_NAV_ITEMS`), My Workspace. Super Admin currently sees all CRM items.
...
- Three cards: **Service** / **Product** / **Both** with icons + description (Bookings vs
  Products + inventory core + supply chain core).
...
- In `Sidebar.jsx` add `ERP_NAV_ITEMS` array:
  - `{ label: 'inventory core', icon: 'inventory', path: '/inventory', external: true }`
  - `{ label: 'supply chain core', icon: 'supplychain', path: '/supply-chain', external: true }`
- Render a new **inventory core** and **supply chain core** group (ERP level, outside Core 360),
